...
`r str_to_title(english::as.english(nrow(experiment_two_DS)))` individuals participated in the present experiment. A majority of the participants in experiment 1 identified as male (*n* = `r gender_results$freq[2]`). Table 1 shows the demographic information for experiment 1. Table 2 presents the results of a Bayesian correlational matrix of all measures. As evidenced in the Bayesian correlational matrix, most surveys positively correlated with one another. 

### Spitefulness

On average, individuals were not rated as being more spiteful, (*M* = `r mean(analysisDF$Spite)`, *SD* = `r sd(analysisDF$Spite)`, Min-max = [`r min(analysisDF$Spite)` - `r max(analysisDF$Spite)`]). Justification as a function of the four indices was moderately explained by the model (*R^2^* = `r bayes.dm5[1]`). 
...
apa_table(
 demographic_table, stub_indents = list("Age" = c(1:2), "Gender" = c(3, 4), "Ethnic Origin" = c(5:12), "Education" = c(13:18), "Ethnicity" = c(19:22)), col.names = c("Demographic Characteristic", ""), cap = "Participant Demographic Information (Experiment 1)"
)
```
